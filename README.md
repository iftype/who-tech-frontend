...
기능 단위(`features/*`) + `lib/api.ts`로 백엔드 계약을 소비합니다. **도메인 엔티티 클래스는 두지 않아도 되고**, API DTO는 `types/`·feature 내부 타입으로 맞추면 됩니다. 복잡한 클라이언트 전용 규칙이 생기면 해당 feature의 순수 함수·소형 타입부터 두는 편이 단순합니다.
